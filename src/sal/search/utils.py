...
def generate_k_steps(
    ...
    gen_results = []
    for i, text in enumerate(templated_convs):  # 所有在这里可以看到的都是 active 的
    ...
    for i in range(lookahead_steps + 1):
        ...
        ]  # 虽然之前可能没有 eos，但是如果生成多次，就可能有了。只需要给那些没有完成的生成结果继续生成
        gen_prompts = [
        ...
        for gen_result, output in zip(current_gen, llm_outputs):  # 输入和输出一一对应
            ...
            gen_result.lookahead_text = gen_result.lookahead_text + gen_text  # 如果有多步，就将刚刚生成的结果放到lookahead_text中，之后会当作前提来使用
