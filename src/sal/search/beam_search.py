...
def _beam_search(batch_of_prompts, config: Config, llm: LLM, prm: PRM) -> list[Beam]:
    ...
    for i in tqdm(range(config.num_iterations), desc="Beam search iterations"):
        ...
        lookahead = 0 if i == config.num_iterations - 1 else config.lookahead  # 如果是最后一步就是 0，否则是 1
        gen_results = generate_k_steps(
            templated_convs, lookahead, llm, sampling_params, 1
        )  # 生成下一步的结果

        prompts, completions = [], []
        for beam, gen_result in zip(active_beams, gen_results, strict=True):  # 对下一步的结果进行赋值
        ...
        agg_scores = [
            [aggregate_scores(s, config.agg_strategy) for s in score]  # 每一次迭代的 s
            for score in scores  # 每一个 beam 的 score
        ]
