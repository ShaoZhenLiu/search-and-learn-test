    ...
    for i in tqdm(range(config.num_iterations), desc="Beam search iterations"):
        ...
        for beam, gen_result in zip(gen_beams, gen_results, strict=True):
            ...
            if len(beam.next_texts) != config.beam_width:
                beam.pruned = True
                # rarely ~1/1000 the model will generate few beams than expected. #: investigate why
                logger.warning(
        ...
        # scoring and chose best generation per beam : add option for selection across beams within the same prompt
