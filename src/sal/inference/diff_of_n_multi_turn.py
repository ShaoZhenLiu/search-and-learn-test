...
def _diff_of_n_multi_turn(batch_of_prompts, config: Config, llm: LLM) -> dict:
    convs = [
        [
    ...
    convs = generate_convs(convs, prompt_index=2)  # 前面只进行了n-1次，这里完成最后一次回答
    convs = generate_convs(convs, prompt_index=3)  # 从前n种中选择k种解法最不相同的
    convs = generate_convs(convs, prompt_index=4)  # 评价k种解法
...
def diff_of_n_multi_turn(examples, config: Config, llm: LLM):
    """
    examples: 根据 config.search_batch_size 调整里面的个数，默认为25
    """
    problems = examples["problem"]
    step_result = _diff_of_n_multi_turn(problems, config, llm)

    # Group together and store in the dataset
